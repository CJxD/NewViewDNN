...
$script $val_flags -x 32 -y 32 -C 3 -d --model-file checkpoints/32x32x3-diff/model.ckpt 
...
$script $val_flags -x 32 -y 32 -C 4 --model-file checkpoints/32x32x4/model.ckpt 
...
$script $val_flags -x 64 -y 64 -C 3 --model-file checkpoints/64x64x3/model.ckpt 
...
$script $val_flags -x 128 -y 128 -C 3 --model-file checkpoints/128x128x3/model.ckpt 
...
$script $val_flags -x 256 -y 256 -C 3 --model-file checkpoints/256x256x3/model.ckpt 
...
$script $val_flags -X 512 -Y 512 -C 3 --model-file checkpoints/512x512x3/model.ckpt 
