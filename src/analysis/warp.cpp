...
void projectTexture()
{
	...
	glTry(glUniformMatrix4fv(loc, 1, GL_FALSE, value_ptr(proj1MVP)));
	...
	glTry(glUniformMatrix4fv(loc, 1, GL_FALSE, value_ptr(proj2MVP)));
}
...
void onKeyPress(GLFWwindow* window, int key, int scancode, int action, int mods)
{
	if (action == GLFW_PRESS || action == GLFW_REPEAT)
	{
		...
		case GLFW_KEY_UP:
			proj1Origin += vec3(0.0f, 0.1f, 0.0f) * length(proj1Origin);
			break;
		case GLFW_KEY_LEFT:
